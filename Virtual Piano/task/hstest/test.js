    ...
    tests = [
        ...
        this.page.execute(() => {
            ...
            if (audioElements === 0) {
                ...
                return wrong(`There are not enough audio objects, ${audioElements} of 12 objects were found`);
            } else if (audioElements > keys.length) {
        ...
        this.page.execute(() => {
            ...
            return len === 2 ?
                correct() :
                wrong(`Div with class 'container' should contain 2 elements, found: ${len}`)
        }),
        ...
        this.page.execute(() => {
            ...
            if (!setsEquals) {
                return wrong(`The names of your keys are incorrect. It must be: A, S, D, F, G, H, J, W, E, T, Y, U`);
            }
        ...
        this.page.execute(() => {
            ...
            for (let elem of this.innerDivElements) {
                ...
                if (elem !== 'div') {
                    return wrong(`Element inside container #${i} is not <div> element, it's <${elem}>`);
                }
            }
            return correct();
        }),
        ...
        this.page.execute(() => {
            ...
            if (whiteKeysElement.length === 0) {
                ...
                return wrong(`Found ${whiteKeysElement.length} elements with class 'white-keys'` +
                    `, the page should contain just a single such element.`);
            }
            ...
            return len === 7 ?
                correct() :
                wrong(`Div with class 'white-keys' should contain 7 elements, found: ${len}`)
        }),
        ...
        this.page.execute(() => {
            ...
            for (let elem of this.innerDivElements) {
                ...
                elem = elem.nodeName.toLowerCase();
                if (elem !== 'kbd') {
                    return wrong(`Element #${i} inside div with class 'white-keys' is not <kbd> element, it's <${elem}>`);
                }
            }
            return correct();
        }),
        ...
        this.page.execute(() => {
            ...
            for (let elem of this.innerDivElements) {
                ...
                elem = elem.innerHTML;
                if (elem.length === 0) {
                    return wrong(`Element #${i} inside div with class 'white-keys' is empty, but should contain a single letter.`);
                } else if (elem.length > 1) {
                    return wrong(`Element #${i} inside div with class 'white-keys' contains ${elem.length} symbols, ` +
        ...
        this.page.execute(() => {
            ...
            if (blackKeysElement.length === 0) {
                ...
                return wrong(`Found ${blackKeysElement.length} elements with class 'black-keys'` +
                    `, the page should contain just a single such element.`);
            }
            ...
            return len === 5 ?
                correct() :
                wrong(`Div with class 'black-keys' should contain 5 elements, found: ${len}`)
        }),
        ...
        this.page.execute(() => {
            ...
            if (distanceET < distanceWE + 1) {
                return wrong(`Are you sure you've positioned the black keys like a real piano?` +
